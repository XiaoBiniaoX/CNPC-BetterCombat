...
public final class NpcAttackSelector {
     ...
     *
     * <p><b>注意：这条路只在服务端可靠</b>（{@link AnimationGroupRegistry#get} 读的
     * {@code RESOLVED} 表只在服务端填充）。客户端要取持握姿态请用
     * {@link #poseFor} / {@link #isTwoHandedPose}。
     */
    @Nullable
     ...
     * 但<b>打开一次 AI 界面就会被填上</b>：
     * <pre>
     * GuiNpcAI 构造器  -> Packets.sendServer(new SPacketMenuGet(AI))       // GuiNpcAI.java:55
     * SPacketMenuGet   -> 服务端回传 ais.save(data)                        // SPacketMenuGet.java:74
     *                  -> 客户端 GuiNpcAI.setGuiData 把它 readToNBT 进客户端实体的 ais  // :121
     * </pre>
     * 于是客户端 {@code ais} 里留下了<b>那一刻的快照</b>。此后脚本在服务端改了动画组、
     * 镜像也随 {@code PacketNpcUpdate} 更新了，若取值顺序写成“先 ais 有值就用”
     * -> 那个<b>过期快照</b>会一直挡在前面 -> 持握姿态永远停在旧值。
     *
     * <p>表现出来就是：攻击动画立刻变新的（走服务端），持握姿态却还是旧的
     * （客户端读到 ais 快照）；再打开一次 AI 界面，快照被刷新，退出来姿态才变
     * —— 看着像“要开 GUI 才生效”，实际是“开 GUI 顺手刷新了那份过期快照”。
     *
    ...
    /**
     * 该实体生效的<b>持握姿态动画名</b>，没有则返回 null。<b>双端可用。</b>
     *
     ...
     *
     * <p>没有用“带 pose 的组一定是双手”这条捷径：虽然官方带 pose 的组
