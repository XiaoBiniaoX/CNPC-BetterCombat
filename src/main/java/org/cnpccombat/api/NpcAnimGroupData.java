 ...
 *       -> 服务端 {@code npc.ais.readToNBT(data)}（SPacketMenuSave.java:101）</li>
 * </ul>
...
public interface NpcAnimGroupData {
     ...
     * {@code EntityNPCInterface.updateClient()}。
     */
    ...
    //
    // 为什么这样就能被脚本调到（已核实 1.21.1 反编译源）：
    //  1. CNPC 的脚本引擎是 **standalone Nashorn（JSR-223）**，不是 GraalJS
    //     （ScriptController.java:82 反射加载
    ...
    //     （NPCWrapper.java:106，中间没有代理层），而 Nashorn 在**实际类**上找方法
    //     -> 我们把方法混入 DataAI，脚本立刻可见，无需改 INPCAi 接口
    //        （该接口不含 save/readToNBT，也不含我们的新方法，但这不影响 Nashorn）。
    //
     ...
     *
     * <p>只包含<b>真正带攻击动作</b>的组（弓弩那种没有 hitbox 的组已被过滤掉），
     * 与 AI 面板里的选择列表完全一致。
     */
