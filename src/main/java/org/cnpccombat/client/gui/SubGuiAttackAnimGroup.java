 ...
 *
 * <p>沿用 CNPC 子界面惯例（对照 {@code SubGuiNpcMovement}）：
 * {@code extends GuiBasic}、背景 menubg.png、256x216、直接修改共享的 DataAI 对象。
 * <b>不自己 save()</b> —— 关闭时 {@code GuiWrapper.close()} 会调用父界面
 * {@code GuiNpcAI.save()}，由它通过 CNPC 原生的
 * {@code SPacketMenuSave(EnumMenuType.AI, ...)} 一并提交。
 *
 * <p>因此没有任何自定义 C2S 包，权限校验完全由 CNPC 负责
 * （{@code SPacketMenuSave} 自带手持魔杖校验 + {@code NPC_AI} 权限节点校验）。
 */
@OnlyIn(Dist.CLIENT)
public class SubGuiAttackAnimGroup extends GuiBasic implements ICustomScrollListener {
    ...
    public void init() {
        ...
        // setList 会原地排序传入的 list，所以给它一份拷贝 ——
        // AnimationGroupRegistry.available() 返回的是不可变视图。
        ...
        this.addButton(new GuiButtonNop((IGuiInterface) this, ID_CLEAR,
                this.guiLeft + 6, this.guiTop + 192, 90, 20, "cnpccombat.gui.clearGroup"));
        this.addButton(new GuiButtonNop((IGuiInterface) this, ID_DONE,
                this.guiLeft + 190, this.guiTop + 192, 60, 20, "gui.done"));
    }
